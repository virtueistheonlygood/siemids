...
2. Enable/start it as a native systemd service (`suricata.service`, shipped by the distro package) -- see [Configure Suricata](#configure-suricata). Then run `scripts/suricata-update-timer-install.sh`, which:
   ...
   - Pulls the rules immediately, then installs the daily-refresh systemd timer (`resources/suricata/suricata-update.service`/`.timer`), scheduled for `05:00 UTC` (`OnCalendar=*-*-* 05:00:00`, plus up to a 30-minute randomized delay). **The timer live-reloads Suricata's rules with no capture gap** -- `suricata-update` auto-detects the running instance via its `unix-command` socket and triggers a reload itself after a successful test, but at this deployment's ~78k-rule size, plain live-reload used to reliably OOM-kill the update's own `suricata -T` test step on a 3.7GB Raspberry Pi 4B (two processes' worth of detection engine at once was more than the box had). Fixed by two changes, both required: (1) an extra 2GB disk-backed swap file (`/var/swap`, activated and persisted by the install script alongside the existing 2GB zram swap) gives the test step enough headroom to survive instead of OOM-killing -- confirmed via dmesg, zero OOM kills with it vs. reliable kills without; (2) `nice -n 19` + `ionice -c3` on the update process so it yields CPU/disk I/O to the live capture threads while swapping -- without this, live capture measurably drops packets during the compile (~38.7k drops smeared over ~9 minutes, per Suricata's own `stats.capture.kernel_drops`), but with it, the same compile added only ~140 drops, close enough to zero to prefer this over stopping Suricata at all.
   - **`abuse.ch/urlhaus` is deliberately not enabled**, even with the stop-first approach -- tested twice, independently: once with Suricata still running (2.6GB+ RSS at kill), once with it stopped first (2.9GB+ RSS at kill, *more* headroom and it still wasn't enough). It looks like just another IoC feed but is actually ~30k content-heavy HTTP rules; compiling it in (the MPM/pattern-matcher build step, not steady-state runtime) needs more memory than this Pi has under any configuration tried so far (confirmed via dmesg both times). Only turn it on if the sensor has meaningfully more RAM than a Pi 4B.
   - **Memory**: `et/open`'s ~40k signatures alone took Suricata from a small baseline to ~1.5GB RSS on a Pi 4B already running AdGuard/WireGuard/PIA -- confirm headroom before enabling it on constrained hardware. By the time all four IoC feeds are added on top, this deployment's compile/test step has essentially no RAM margin left; the extra swap file and nice/ionice above are what make a same-box live reload possible at all, and even then a real (if now negligible) capture cost remains during each update.
3. Run `scripts/interfaces.py` **on the sensor host** first -- it generates the `geoip/*.json` and `env/*.env` files Device B's Filebeat geo-enrichment processors read from. Edit it to set `localnet_1`/`vpnet_1` (and `piavpn_1`, if the sensor has a second VPN egress like a PIA tunnel) to that host's actual interfaces:
  ...
  ```
   ...
   variable. *We are planning to automate this step by identifying active network
   interfaces.*
...
![Events Dashboard](resources/images/screenshot5.png)
...
![Events Dashboard](resources/images/screenshot1.png)
![Alert Dashboard](resources/images/screenshot2.png)
![Logs Dashboard](resources/images/screenshot3.png)
![DNS Dashboard](resources/images/screenshot4.png)
